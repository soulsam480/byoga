...
// ? === UPI spend ===
export const FOOD_RE = /^food\s(?<tag_cat>\w+)|(?<tag>food|fod|fpod|foos|dood|fruit|coffe|lunch|dinner|juice|sweets|curd|chicken|mutton|milk|egg|coke|coconut|choco[a-z]+|iron\shill|swiggy|zomato)/i

export const BIKE_RE = /^bike\s(?<tag_cat>\w+)|(?<tag>bike|motorcycle|suzuki|parking|balaklava)/i
...
export const GROCERY_RE = /^grocery\s(?<tag_cat>\w+)|(?<tag>grocery|vegetable|bag|polythene)/i

export const TRANSPORT_RE
  = /(?<tag>transport|taxi|bus|fare|cab|uber|rapido|cleartrip)/i

export const MEDICAL_RE = /^medical\s(?<tag_cat>\w+)|(?<tag>medicine|medical|health|check up)/i
...
export const PERSONAL_RE = /(?:(?:personal|shopping)\s(?<tag_cat>\w+))?(?<tag>clothes|decathlon|slipper|clothing|shopping|allowance|stuff)/i

export const CASH_TRANSFER_RE = /^transfer\s(?<tag_cat>\w+)|(?<tag>atm\scash|cash|transfer|refund|lend)/i
