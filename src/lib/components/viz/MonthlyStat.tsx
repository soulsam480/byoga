...
export function AllTimeMonthlyViz() {
  const { value } = useQuery(['all_time_monthly_stat'], async () => {
    ...
    const results = await db
      ...
      .select(eb => [
        ...
        sql<string>`strftime('%m-%Y', ${eb.ref('transaction_at')})`.as(
          'transaction_month'
        ),
        // monthly balance as of 30th of the month
        eb
          .selectFrom('transactions as t2')
          .select('balance')
          .where(
            in_eb => sql`strftime('%m-%Y', ${in_eb.ref('t2.transaction_at')})`,
            '=',
            sql`strftime('%m-%Y', ${eb.ref('transactions.transaction_at')})`
          )
          .where(
            in_eb => sql`strftime('%e', ${in_eb.ref('t2.transaction_at')})`,
            '<',
            '30'
          )
          .orderBy(
        ...
        eb.fn
          ...
          .filterWhere(eb =>
            eb.and([
              eb('credit', 'is', null),
              eb('transaction_category', 'not in', [
                'deposit',
                'salary',
  ...
  const months = useComputed(
    () => value.value?.map(it => it.transaction_month) ?? []
  )
  ...
  return (
    <div className='monthly-stat border flex flex-col gap-4 border-base-200 rounded-lg p-4'>
      <div className='flex justify-between'>
        <div className='text-sm font-semibold'>Monthly Statistics</div>
      ...
      <Chart data={pois.value} class='h-72 monthly-stat-viz'>
        ...
        <LinearAxis
          ...
          max={400000}
          major
          labels={value => formatCurrency(value)}
        />

        <Highlight
          class='budget-highlight-section'
          limits={{ y1: 0, y2: monthlyBudget.value }}
        />

        <Highlight
          class='investment-highlight-section'
          limits={{ y1: 0, y2: monthlyInvestment.value }}
        />
      ...
      <div class='flex flex-col gap-1'>
        ...
        <div className='inline-flex gap-4'>
          <span class='text-xs inline-flex items-center gap-1'>
            <CarbonDotMark class='text-expense' /> Monthly Budget:{' '}
            {formatCurrency(monthlyBudget.value)}
          </span>

          <span class='text-xs inline-flex items-center gap-1'>
            <CarbonDotMark class='text-investment' /> Monthly Investment Goal:{' '}
